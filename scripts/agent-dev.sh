...
# "roost-dev" (the repo name plus the "-dev" suffix).
...
# than adding a second one - a stale "roost-dev" is overwritten by
...
PROJECT_NAME="roost"
AGENT_NAME="roost-dev"
WORKSPACE="/workspaces/roost"
...
if [ -x "/workspaces/roost/scripts/agent-dev.sh" ]; then
  "/workspaces/roost/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
